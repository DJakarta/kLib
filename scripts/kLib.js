...
kLib={
	...
	extendDefaultObjectMethods: function () {
		...
		Array.prototype.insert=function (position) {
			...
			//add the position and how many for splice
			args.unshift(0);
	...
	dom: {
		...
		addClass: function () {
			var i, len=this.length, element, string;
			...
			for (i=0; i<arguments.length; i++) {
				...
				//for each element, if it can have class, add the class to the className
				for (i=0; i<len; i++) {
					element=this[i];
					if ((typeof element.className==='string') || (element.className instanceof String)) {
		...
		//remove an existing class of an element
		removeClass: function () {
			var i, j, k, strings, len=this.length, element, exp;
			...
			for (i=0; i<arguments.length; i++) {
				...
				//for each class in argument
				for (j=0; j<string.length; j++) {
					
					//make a regular expression for global search
					exp=new RegExp(string[i], 'g');
					
					//for each element, if it can have class, remove the class from the className
					for (k=0; k<len; k++) {
						element=this[k];
						if ((typeof element.className==='string') || (element.className instanceof String)) {
	...
	debug: {
		writeToConsole: function (newText, overWriting) {
			...
			//create the display otherwise
				...
				consoleDisplay.style.width='200px';
